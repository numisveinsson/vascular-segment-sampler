...
Usage:
	python -m analysis.compute_metrics_meshes /path/to/gt_dir /path/to/pred_dir \
			--out-csv results.csv

Example:
	# basic run, write CSV
	python -m analysis.compute_metrics_meshes /data/gt_vtps /data/pred_vtps --out-csv metrics.csv
...
# Import optional functionality with independent fallbacks.
...
_clipping_import_error = None
_volume_radii_import_error = None
_dice_radii_import_error = None
...
try:
	...
	_clipping_import_error = str(e_local)
...
try:
	...
	_volume_radii_import_error = str(e_local)
...
try:
	...
	_dice_radii_import_error = str(e_local)
	dice_per_radii = None
	get_dice_per_radii_fieldnames = None

if _clipping_import_error is not None:
	print(
		"Warning: clipping imports failed. "
		f"Reason: {_clipping_import_error}",
		file=sys.stderr,
	)
if _volume_radii_import_error is not None:
	print(
		"Warning: volume_radii imports failed. "
		f"Reason: {_volume_radii_import_error}",
		file=sys.stderr,
	)
if _dice_radii_import_error is not None:
	print(
		"Warning: dice_radii imports failed. "
		f"Reason: {_dice_radii_import_error}",
		file=sys.stderr,
	)
...
def compute_metrics(gt_poly: vtk.vtkPolyData, pred_poly: vtk.vtkPolyData, max_points: Optional[int] = None, voxel_spacing: Optional[Tuple[float, float, float]] = None, centerline: Optional[vtk.vtkPolyData] = None, metrics_to_compute: Optional[frozenset] = None, min_seg: int = 300) -> dict:
	...
	if centerline is not None and (do_centerline_overlap or do_volume_radii or do_dice_radii):
		...
		if do_volume_radii and volume_error_per_radii is not None:
			try:
				vpr = volume_error_per_radii(gt_poly, pred_poly, centerline, min_seg=min_seg)
		...
		if do_dice_radii and dice_per_radii is not None:
			try:
				dpr = dice_per_radii(gt_poly, pred_poly, centerline, spacing=voxel_spacing, min_seg=min_seg)
...
def main(argv: Optional[List[str]] = None) -> int:
	...
	gt_dir = args.gt_dir
	pred_dir = getattr(args, 'pred_dir_opt', None)
	...
	if metrics_to_compute is not None:
		if 'dice_radii' in metrics_to_compute and dice_per_radii is None:
			print(
				"Error: metric 'dice_radii' requested but dice_per_radii import failed. "
				f"Import reason: {_dice_radii_import_error}",
				file=sys.stderr,
			)
			return 1
		if 'volume_radii' in metrics_to_compute and volume_error_per_radii is None:
			print(
				"Error: metric 'volume_radii' requested but volume_error_per_radii import failed. "
				f"Import reason: {_volume_radii_import_error}",
				file=sys.stderr,
			)
			return 1

	# Validate input mode: either single pred_dir or --predictions-root
	if predictions_root is None and pred_dir is None:
		print('Error: provide --pred-dir or set --predictions-root', file=sys.stderr)
		return 1

	# Single output directory for all generated files
	output_base = out_dir_arg if out_dir_arg is not None else (predictions_root if predictions_root is not None else pred_dir)
	if output_base is None:
		print('Error: could not determine output directory', file=sys.stderr)
		return 1
	os.makedirs(output_base, exist_ok=True)

	# Route all output artifacts through output_base
	if out_csv is None:
		out_csv = os.path.join(output_base, 'mesh_metrics.csv')
	else:
		out_csv = os.path.join(output_base, os.path.basename(out_csv))
	if summary_csv is None and predictions_root is not None:
		...
		summary_csv = os.path.join(output_base, os.path.basename(summary_csv))
	if clip:
...
if __name__ == '__main__':
	"""
	python analysis/compute_metrics_meshes_comparison.py \
  	...
  	--centerline-dir "/Users/nsveinsson/Documents/datasets/vmr/vmr_all/centerlines" \
  	--metrics dice_radii \
  	--out-dir /Users/nsveinsson/Documents/datasets/vmr/vmr_all \
	--spacing 1.0 \
	--min-seg 10

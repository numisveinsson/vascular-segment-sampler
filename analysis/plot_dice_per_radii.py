"""Plot dice-per-radii metrics from summary.csv (from compute_metrics_meshes_comparison.py).

Creates grouped bar plots: x-axis = radii buckets, with each bucket having one bar
per method. Legend shows methods.

Usage:
	python -m analysis.plot_dice_per_radii summary.csv
	python -m analysis.plot_dice_per_radii summary.csv --out dice_radii.png
"""
...
def _find_dice_radii_columns(df: pd.DataFrame) -> list[str]:
	"""Find dice-per-radii columns in the summary DataFrame.
...
def _extract_bucket_label(col: str) -> str:
...
def plot_dice_per_radii(
	...
	"""Plot dice-per-radii as grouped bar chart.

	X-axis: radius buckets. Each bucket has one bar per method. Legend: methods.
	...
	cols = _find_dice_radii_columns(df)
	if not cols:
		raise ValueError(
			"No dice-per-radii columns found. "
			"Ensure summary.csv was generated with --centerline-dir and dice_radii metrics."
		)
	...
	bucket_labels = [_extract_bucket_label(c) for c in cols]
	...
	# Bar layout: x-axis = buckets, grouped bars = methods
	bar_width = 0.8 / n_methods
	x = np.arange(n_buckets)
	offsets = np.linspace(-0.4 + bar_width / 2, 0.4 - bar_width / 2, n_methods)
	...
	for i, method_display in enumerate(methods):
		vals = data[i, :]  # one value per bucket for this method
		err = std_data[i, :] if has_std and std_data is not None else None
		ax.bar(
			x + offsets[i],
			vals,
			bar_width,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
	p = argparse.ArgumentParser(
		description='Plot dice-per-radii from summary.csv: x-axis=buckets, grouped bars=methods'
	)
	p.add_argument(
		'summary_csv',

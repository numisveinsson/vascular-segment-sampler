...
def add_wilcoxon_signed_rank_bracket(
    ...
    vals1: np.ndarray,
    vals2: np.ndarray,
    patients1: np.ndarray,
    patients2: np.ndarray,
    pos1: float,
    pos2: float,
) -> None:
    """Run Wilcoxon signed-rank (paired) test on patient-matched values and draw bracket.

    Aligns vals1 and vals2 by patient; uses only patients present in both groups.
    """
    # Align by patient (inner join)
